...
export function registerRoutes(app: Express) {
  ...
  app.get("/api/admin/dashboard", requireAuth, requireAdmin, async (_req: Request, res: Response) => {
    try {
      ...
      const totalRevenue = (allOrders || []).reduce((sum: number, o: any) => sum + (Number(o.total_amount) || 0), 0);

      const { data: reviews } = await supabase.from('reviews').select('rating');
      const avgRating = reviews && reviews.length > 0
        ? Math.round((reviews.reduce((sum: number, r: any) => sum + (Number(r.rating) || 0), 0) / reviews.length) * 10) / 10
        : 0;
      ...
      const growthPercent = (lastWeekOrders && lastWeekOrders > 0)
        ? Math.round(((thisWeekOrders || 0) - lastWeekOrders) / lastWeekOrders * 100)
        : 0;
